...
def load_mutations(mutations_path: str, samplesheet_path: str) -> pd.DataFrame:
    muts = _do_load_mutations(mutations_path)
    df = muts[muts['clone'].notna()]
    muts = _annotate_tissue_cohort(muts, samplesheet_path)
    mask1 = muts['vclass'].isin(['SNV', 'INDEL'])
    mask2 = muts['vclass'].isin(['SV', 'CNA'])
    muts.loc[mask1, 'ID'] = muts.loc[mask1].apply(lambda x: f"{x['donor']}|{x['vclass']}|{x['coords']}", axis=1)
    muts.loc[mask2, 'ID'] = muts.loc[mask2].apply(lambda x: f"{x['donor']}|{x['vclass']}|{x['gene']}", axis=1)
    assert muts['ID'].isna().sum() == 0
    return muts

def _do_load_mutations(mutations_path: str) -> pd.DataFrame:
...
def _annotate_tissue_cohort(table: pd.DataFrame, samplesheet_path: str) -> pd.DataFrame:
    df = table.copy()
